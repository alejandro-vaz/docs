...
<Steps>
    ...
    <Step stepNumber="4">
        <AccordionGroup>
            ...
            <Accordion title="v4.10.5">
                ## Major
                1. Moved styling to *Tailwind CSS* + *Motion,* removing *SASS.*
                1. Switched from a `POST`-based *API* to *WebSockets.*
                1. *API* now does not touch *SQL* code, but rather calls procedures.
                1. Integrated *React Bits,* alongside a new background for `&`.
                1. Integrated *React Router* for smooth transition between pages.
                ## Minor
                1. Deleted unused database tables and endpoints.
                1. Removed dir hacking and `include()` from `API.py`.
                1. Moved *Python* imports under `æ` from `__init__`.
                1. Added a features table for feature-requesting.
                1. Refactored `database.py` extension to only allow for procedure calling.
                1. Remade `$Playground`, now being smooth and using `via`s (*WebSockets*).
                1. Trimmed down `index.html` thanks to *Tailwind CSS,* and also removed `€` extern especifiers.
                1. Created a *CTA* button.
                1. Added a friendly message to the `/feature` view asserting that the site is under development.
                1. Removed types for everything except for *API* requests and responses.
                ## Patch
                1. Downgraded to *Mathsys* stable `0.10.2`.
                1. Switched from `.ttf` font to `.woff`.
                1. Updated `sitemap.xml`.
                1. Added *Nginx* support for *WebSockets.*
                1. Components are now all imported by `ß` and re-exported for ease of use.
                1. `ß` components now are located under family-type folders.
                1. `ß` now re-exports common libraries like *React.*
                1. Improved *SQL* syntax with uppercase and lowercase by capitalizing.
                1. Stripped down `README.md` since documentation is now available at https://docs.abscissa.eu.
                1. Added support for users using `@abscissa.eu` mails, like `root` has now.
            </Accordion>
        </AccordionGroup>
    </Step>
</Steps>